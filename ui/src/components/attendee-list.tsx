...
export type AttendeeListProps = InferProps<typeof AttendeeListPropTypes> &
  AttendeeListProp;

export const AttendeeList: FC<AttendeeListProps> = ({ attendees }) => {
  ...
  if (screens.lg) {
    const columns = [
      ...
      },
      {
        ...
        key: 'dateConfirmed',
        render: (dateConfirmed?: Date) => (
          <>{dateConfirmed ? dateConfirmed.toISOString() : 'Not Registered'}</>
        ),
      },
    ];
    return (
      <>
        <Table dataSource={attendees as any} columns={columns}></Table>
      </>
    );
  } else {
    return (
            ...
            <List.Item>
              <Card title={`${attendee.firstName} ${attendee.lastName}`}>
                <Descriptions>
                  ...
                  <Descriptions.Item label="DateConfirmed">
                    {attendee.dateConfirmed
                      ? attendee.dateConfirmed.toISOString()
                      : 'Not Registered'}
                  </Descriptions.Item>
                </Descriptions>
              </Card>
